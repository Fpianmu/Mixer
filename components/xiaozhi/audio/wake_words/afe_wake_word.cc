...
void AfeWakeWord::Feed(const std::vector<int16_t>& data) {
    ...
    if (++g_feed_count % 50 == 0) ESP_LOGI("AFE", "Feed #%d, size=%d", g_feed_count, (int)data.size());
...
void AfeWakeWord::AudioDetectionTask() {
    ...
    while (true) {
        ...
        if (++fetch_count % 20 == 0) ESP_LOGI("AFE", "Fetch #%d, wake_state=%d", fetch_count, res->wakeup_state);

        StoreWakeWordData(res->data, res->data_size / sizeof(int16_t));

        if (res->wakeup_state == WAKENET_DETECTED) {
            ESP_LOGI("AFE", "===== WAKENET_DETECTED! =====");
            Stop();
