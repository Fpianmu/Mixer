...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    ...
    for (int i = 0; i < samples; i++) {
        ...
        if (input_gain_ > 0.01f) {
            value = (int32_t)(value * input_gain_);
        }
        dest[i] = (value > INT16_MAX) ? INT16_MAX : (value < -INT16_MAX) ? -INT16_MAX : (int16_t)value;
    }
    // Print peak every 50 calls (debug)
    static int peak_cnt = 0;
    if (++peak_cnt % 50 == 0) {
        int16_t pk = 0;
        for (int i = 0; i < samples; i++) if (abs(dest[i]) > pk) pk = abs(dest[i]);
        ESP_LOGI("I2S", "PCM peak=%d (gain=%.1f)", pk, input_gain_);
    }
    return samples;
}
...
void NoAudioCodec::EnableInput(bool enable) {
    ...
    if (enable == input_enabled_) {
        ESP_LOGD("I2S", "EnableInput(%d) — already in that state, skipping", enable);
        return;
    }
    ESP_LOGI("I2S", "EnableInput(%d)", enable);
    if (enable) {
