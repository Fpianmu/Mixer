...
void AudioService::Start() {
    ESP_LOGI("AUDIO", "AudioService::Start() called, creating tasks...");
    service_stopped_ = false;
    xEventGroupClearBits(event_group_, AS_EVENT_AUDIO_TESTING_RUNNING | AS_EVENT_WAKE_WORD_RUNNING | AS_EVENT_AUDIO_PROCESSOR_RUNNING);

    // esp_timer_start_periodic disabled (crash bug);
    ...
    xTaskCreatePinnedToCore([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "audio_input", 4096 * 2, this, 8, &audio_input_task_handle_, 0);
    ...
    xTaskCreate([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "audio_output", 4096 * 2, this, 4, &audio_output_task_handle_);
#else
    /* Start the audio input task */
    xTaskCreate([](void* arg) {
        AudioService* audio_service = (AudioService*)arg;
        audio_service->AudioInputTask();
        vTaskDelete(NULL);
    }, "audio_input", 4096 * 2, this, 8, &audio_input_task_handle_);
    ...
    xTaskCreate([](void* arg) {
        ...
        audio_service->AudioOutputTask();
        vTaskDelete(NULL);
    }, "audio_output", 4096, this, 4, &audio_output_task_handle_);
#endif
    ...
    xTaskCreate([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "opus_codec", 4096 * 10, this, 2, &opus_codec_task_handle_);
}

void AudioService::Stop() {
    // esp_timer_stop disabled
...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    if (!codec_->input_enabled()) {
        // esp_timer_stop disabled
        // esp_timer_start_periodic disabled
...
static int g_input_task_runs = 0;
void AudioService::AudioInputTask() {
    ESP_LOGI("AUDIO", "AudioInputTask started");
    while (true) {
        if (++g_input_task_runs == 100) {
            ESP_LOGI("AUDIO", "Task loop #%d, waiting for events (bits=0x%x)",
                     g_input_task_runs, (int)xEventGroupGetBits(event_group_));
        }
        ...
        if (g_input_task_runs >= 100) {
            ESP_LOGI("AUDIO", "Woke up! bits=0x%x, serving...", (int)bits);
            g_input_task_runs = 0;
        }
        if (service_stopped_) {
            break;
...
#if CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32P4
    if (false && esp_srmodel_filter(models_list_, ESP_MN_PREFIX, NULL) != nullptr) {
